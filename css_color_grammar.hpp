...
//    mapnik::color,
//    (obj.red(), obj.set_red(val))
//    (obj.green(), obj.set_green(val))
//    (obj.blue(), obj.set_blue(val))
//    (obj.alpha(), obj.set_alpha(val))
//    )
...
x3::rule<class rgb_color_percent, mapnik::color> const rgb_color_percent("rgb_color_percent");
x3::rule<class rgba_color_percent, mapnik::color> const rgba_color_percent("rgba_color_percent");
//x3::rule<class hsl_color, mapnik::color> const hsl_color("hsl_color");
...
{
    std::cerr << typeid(_attr(ctx)).name() << std::endl;
    ...
    uint8_t opacity = uint8_t((255.0 * clip_opacity::call(std::get<3>(_attr(ctx)))) + 0.5);
...
auto const css_color_def = no_case[named_colors]
    |
    hex2_color
    |
    hex1_color
